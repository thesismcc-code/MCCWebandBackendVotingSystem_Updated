    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:400px;">
        <div class="modal-content">
            <div class="modal-body text-center py-5 px-4">
                ...
                <p style="color:#6b7280;font-size:14px;">{{ session('success') }}</p>
                @endif
                @if(session('error'))
                ...
                <p style="color:#6b7280;font-size:14px;">{{ session('error') }}</p>
                @endif
            </div>
...
<div class="d-flex align-items-center gap-3">
    ...
    <div>
        ...
        <p style="font-size:12px;color:rgba(255,255,255,.55);font-weight:600;margin:3px 0 0;letter-spacing:.02em;">
            Active Election: {{ $activeElectionName }}
        </p>
        @endif
    </div>
</div>
    ...
    <div class="row g-3">
        <div class="col-md-4">
            <div class="stat-card">
                ...
                <div><div class="stat-num">{{ $totalCands }}</div><div class="stat-lbl">Total Candidates</div></div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="stat-card">
                ...
                <div><div class="stat-num">{{ $positions_filled }}</div><div class="stat-lbl">Positions Filled</div></div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="stat-card">
                ...
                <div><div class="stat-num">{{ count($positions) }}</div><div class="stat-lbl">Total Positions</div></div>
            </div>
        </div>
    </div>
    ...
    <div class="table-card">
        ...
        <div class="toolbar">
            ...
            <select class="filter-select" id="posFilter" onchange="filterCandidates()">
                ...
                @foreach(array_unique(array_column($candidates, 'position')) as $pos)
                <option value="{{ $pos }}">{{ $pos }}</option>
                @endforeach
            </select>
            <span style="font-size:12px;font-weight:600;color:#9ab09a;" id="resultCount">{{ $totalCands }} candidates</span>
        </div>
            ...
            <table class="tbl">
                ...
                <tbody id="candTableBody">
                    ...
                    <tr class="cand-row"
                        ...
                        <td style="padding-left:20px;color:#9ab09a;font-weight:700;font-size:13px;">{{ $i + 1 }}</td>
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                @if($candidate['image'])
                                <img src="/{{ $candidate['image'] }}" class="cand-avatar" alt="{{ $candidate['name'] }}">
                                @else
                                <div class="cand-avatar-placeholder">{{ strtoupper(substr($candidate['name'], 0, 1)) }}</div>
                                @endif
                                <span style="font-weight:700;color:#111827;font-size:14px;">{{ $candidate['name'] }}</span>
                            </div>
                        </td>
                        <td><span class="pos-badge">{{ $candidate['position'] }}</span></td>
                        <td style="color:#6b7280;font-size:13px;">{{ $candidate['course'] ?: '—' }}</td>
                        <td style="color:#6b7280;font-size:13px;">{{ $candidate['year'] ?: '—' }}</td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                <button class="btn-edit btn-edit-candidate"
                                    ...
                                    data-candidate-year="{{ $candidate['year'] }}"
                                    data-candidate-image="{{ $candidate['image'] }}"
                                    title="Edit">
                    ...
                    <tr><td colspan="6">
                        <div class="empty-state">
                            ...
                            <span>@if($activeElectionId) Click the + button to add candidates for this election @else Set up an active election first @endif</span>
                        </div>
                    </td></tr>
                    @endforelse
                </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" style="max-width:660px;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addCandidateForm" action="{{ route('candidate.add') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
<div class="modal fade" id="editCandidateModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" style="max-width:660px;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editCandidateForm" action="{{ route('candidate.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
